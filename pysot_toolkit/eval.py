...
from pysot_toolkit.toolkit.datasets import OTBDataset, UAVDataset, LaSOTDataset, VOTDataset, NFSDataset, VOTLTDataset, TNL2KDataset, LaSOT_extension_subset
...
parser.add_argument('--dataset', '-d', type=str, default='LaSOT',
                    help='dataset name')
# OTB100 NFS  LaSOT     LaSOT_extension_subset  TrackingNet  TNL2k
parser.add_argument('--num', '-n', default=1, type=int,
                    help='number of thread to eval')
parser.add_argument('--tracker_prefix', '-t', default='siamft_train_cfa_smca_attnscale_mlp5',
    ...
    if 'OTB' in args.dataset:
        ...
        if args.vis:
            draw_success_precision(success_ret,
                                   ...
                                   precision_ret=precision_ret,
                                   norm_precision_ret=norm_precision_ret)
    elif 'LaSOT_extension_subset' == args.dataset:
        dataset = LaSOT_extension_subset(args.dataset, root)
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        with Pool(processes=args.num) as pool:
