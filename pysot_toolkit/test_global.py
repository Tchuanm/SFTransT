...
from pysot_toolkit.trackers.tracker_global import Tracker
from pysot_toolkit.trackers.net_wrappers import NetWithBackbone

# python test_global.py --dataset LaSOT --cuda 5 --epoch 288  --win 0.49
parser = argparse.ArgumentParser(description='sftranst pysot tracking')
parser.add_argument('--dataset', type=str, default='LaSOT', help='datasets')
#  LaSOT GOT-10k TrackingNet OTB100 NFS  LaSOT_extension_subset  TNL2k
parser.add_argument('--video', default='', type=str,
                    help='eval one special video')
parser.add_argument('--vis', action='store_true', default=False,  # False  True
                    help='whether visualzie result')
parser.add_argument('--name', default='sftranst', type=str,
                    help='name of results')
parser.add_argument('--cuda', default='2', type=str, help='cuda number')
parser.add_argument('--epoch', default=429, type=int, help='choose which epoch to test')
...
parser.add_argument('--fail_count', default='5', type=int, help='fail frame count')
...
def main():
    ...
    dataset_root = current_path + '/testing_dataset/{}'.format(args.dataset)
    net_path = current_path + '/../ltr/checkpoints/ltr/sftranst/' + args.folds + '/sftranst_ep{}.pth.tar'.format(args.epoch)
    args.name = args.folds + "%d_win%.2f_global_%.2f_count%d" % (args.epoch, args.win, args.thr, args.fail_count)
    # create model
    net = NetWithBackbone(net_path=net_path, use_gpu=True)
    tracker = Tracker(name=args.name, net=net, instance_size=256, search_factor=4,          # 320 5
                      temp_factor=2, exemplar_size=128, window_penalty=args.win, global_thr=args.thr, fail_count=args.fail_count)
    # create dataset
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == 0:
                    ...
                    outputs, global_flag, failure_num = tracker.track(img, global_flag, failure_num)
                    pred_bbox = outputs['target_bbox']
                    pred_bboxes.append(pred_bbox)
