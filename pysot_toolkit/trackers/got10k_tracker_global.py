...
class got10k_Tracker(object):
    def __init__(self, name, net, instance_size=256, search_factor=4, exemplar_size=128, temp_factor=2,
                 window_penalty=0.49, global_thr=0.9, fail_count=10):
        self.name = name
        self.net = net
    ...
    @torch.no_grad()
    def update(self, image, global_flag, Failure_num, idx):
        # calculate x crop size
        if global_flag:
            h_x, w_x, _ = image.shape
            center_pos = np.array([w_x / 2, h_x / 2])
            s_x = math.ceil(math.sqrt(w_x * h_x))
            channel_average = np.mean(image, axis=(0, 1))
            x_crop = self.get_subwindow(image, center_pos, self.instance_size, round(s_x), channel_average)
        else:
        ...
        # cv2.imwrite('./trackers/attention/x_crop_tmp.png', tmp)
        # cv2.waitKey(1)
        ...
        # # score map attn
        # featuremap = score.reshape(1, 32, 32)
        # getCAM2(featuremap, tmp, idx)
        ...
        if pscore[best_idx] < self.global_thr:
            Failure_num += 1
            global_flag = False
            if Failure_num == self.fail_count:
                global_flag = True
                Failure_num = 0
                print('Fail continuously 5 times, Global searching.')
        else:
            Failure_num = 0
            global_flag = False

        return bbox, global_flag, Failure_num
    ...
    def track(self, img_files, box, visualize=True):
        ...
        for f, img_file in enumerate(img_files):
            ...
            if f == 0:
                ...
                boxes[f, :], global_flag, Failure_num = self.update(img, global_flag, Failure_num, idx=f)
