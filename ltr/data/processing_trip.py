...
class BaseProcessing:
    ...
    def __init__(self, transform=transforms.ToTensor(), search_transform=None, template_transform=None,
        ...
        self.transform = {'search': transform if search_transform is None else search_transform,
                          ...
                          'template2': transform if template_transform is None else template_transform,
...
class trackingProcessing(BaseProcessing):
    """ The processing class used for training. The images are processed in the following way.
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['template_images'], data['template_anno'] = self.transform['joint'](image=data['template_images'],
                                                                                     ...
                                                                                     new_roll=False)
            data['template2_images'], data['template2_anno'] = self.transform['joint'](image=data['template2_images'],
                                                                                       bbox=data['template2_anno'],
                                                                                       new_roll=False)

        for s in ['search', 'template', 'template2']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num search/template/template2 frames must be 1"
        ...
        data['template2_images'] = data['template2_images'].squeeze()
        data['search_images'] = data['search_images'].squeeze()
        data['template_anno'] = data['template_anno'].squeeze()
        data['template2_anno'] = data['template2_anno'].squeeze()
