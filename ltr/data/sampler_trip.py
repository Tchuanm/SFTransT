...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    tuple consisting of a set of template frames and search frames, used to train the SFTransT model.
    ...
    def __getitem__(self, index):
        ...
        if is_video_dataset:
            ...
            if self.frame_sample_mode == 'interval':
                ...
                while search_frame_ids is None:
                    ...
                    # if random.random() > 1:  # 0.9 prob for same image template
                    #     same_frame = False
                    #     template2_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                ...
                while search_frame_ids is None:
                    ...
                    # if random.random() > 0.9:  # a quarter prob for same image template,only for got-10k training.
                    #     same_frame = False
                    #     template2_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1,
        ...
        # if not same_frame:
        #     template2_frames, template2_anno, meta_obj_template2 = dataset.get_frames(seq_id, template2_frame_ids, seq_info_dict)
        # else:
        template2_frames, template2_anno, meta_obj_template2 = template_frames, template_anno, meta_obj_template

        data = TensorDict({'template_images': template_frames,
                           'template_anno': template_anno['bbox'],
                           'template2_images': template2_frames,
                           'template2_anno': template2_anno['bbox'],
